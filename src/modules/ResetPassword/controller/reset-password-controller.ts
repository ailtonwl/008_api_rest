...
class ResetPasswordController {
  public async validadeUser(req: Request, res: Response) {
    ...
    try {
      return res.json({
        ...
        data: await resetPasswordService.validadeUser(email),
      });
    } catch (err: any) {
  ...
  public async validadeSecurityCode(req: Request, res: Response) {
    ...
    try {
      return res.json({
        message: ECrud.READ,
        data: await resetPasswordService.validadeSecurityCode(
          email,
          Number(secret),
        ),
      });
